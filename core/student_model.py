    ...
    def update_student_profile(self, student_id: str, interaction_data: Dict[str, Any]):
        if student_id not in self.student_profiles:
            self.student_profiles[student_id] = {
                'knowledge_state': np.zeros(len(self.concept_mapping)) if self.concept_mapping else np.zeros(10),
        ...
        concepts = interaction_data.get('concepts', [concept_id] if concept_id else [])
        performance = interaction_data.get('performance', 0.5)
        content_type = interaction_data.get('content_type')
        time_spent = interaction_data.get('time_spent', 300)
        
        for c in concepts:
            if c:
                current_c_val = profile['concept_mastery'].get(c, 0.4)
                updated_c_val = self.update_knowledge(current_c_val, performance, time_spent)
                profile['concept_mastery'][c] = updated_c_val
                if c in self.concept_mapping:
                    concept_idx = self.concept_mapping[c]
                    if concept_idx < len(profile['knowledge_state']):
                        profile['knowledge_state'][concept_idx] = updated_c_val
